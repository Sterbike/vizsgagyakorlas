...
app.post("/fesztivalhozzaad", async (req, res) => {
  ...
  if (!nev || !szuletesi_datum || !telefonszam || !email || !foglalt_napok_szama || !osszeg) {
    return res.status(400).json({ msg: "Minden mező megadása kötelező!" });
  }
  ...
  try {
    ...
    res.status(200).json({ msg: "Az adatok sikeresen feltöltésre kerültek az adatbázisba!" });
  } catch (error) {
    res.status(400).json({ msg: "Valami hiba történt: " + error.message });
  }
});
...
app.delete("/fesztivaltorol", async (req, res) => {
  try {
    const { id } = req.body;
    const toroltAdat = await Festival.findByIdAndDelete(id);
    if (toroltAdat) {
